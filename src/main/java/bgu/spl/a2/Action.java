...
public abstract class Action<R> {
     ...
     */
   /*package*/
    final void handle(ActorThreadPool pool, final String actorId, final PrivateState actorState) {
        if (!started) {
                actorState.addRecord(actionName);
            this.actorId = actorId;
            this.myPS = actorState;
            this.pool = pool;
            start();
        } else {
            if (myCallback!=null)
    ...
    protected final void then(Collection<? extends Action<?>> actions, callback callback) {
        myCallback = callback;
        if (actions.isEmpty()) {
            sendMessage(this, actorId, myPS);
        }
            int numOfAct = actions.size();
            CountDownLatch count = new CountDownLatch(numOfAct);
